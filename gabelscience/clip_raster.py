...
class ClipAraster:
    ...
    def _init_raster_specs(self):
        ...
        return {"input_raster": create_raster_specs_from_path(self.raw_raster)}
    ...
    def import_polygons(self):
        ...
        grid_gdf, n_cells, grid_features = create_regular_grid(gdf, n_cells=(2, 2),
                                                               ...
                                                               overlap=False)
        grid_gdf.to_file(os.path.join(os.path.split(self.input_pg_file)[0], "grid.shp"), engine="fiona", index=True)
...
if __name__ == "__main__":
    input_shape = r"A:\Iowa_1A\02_mapping\Grids_Lower_Cedar\script_testing\clipper_01.shp"
    epsg = 3418
    value = 0.01
    raster_file = r"A:\Iowa_1A\02_mapping\Grids_Lower_Cedar\script_testing\source_01pct.tif"
    operation_type = "both"  # ["make mask", "clip only", "make mask"]
    alt_output_filename = "test_clip_01"
    exact_or_touched = True

    # operation_type = literals.OpsType(operation_type)
