...
class CreatestatenfhlLinks:
    def __init__(self, extra_params=None):
        ...
        self.cid_excel = Path(__file__).parent.parent / '_metadata' / 'fema_nfip_communities.xlsx'
        self.rel_outpath = Path(__file__).parent.parent / '_metadata'
        self.scratch_dir = self.rel_outpath / '_json_scratch'
        self.rel_outpath.mkdir(parents=True, exist_ok=True)
        self.scratch_dir.mkdir(parents=True, exist_ok=True)
        self.output_file = self.rel_outpath / 'state_nfhl_links.xlsx'
        self.extra_params = extra_params or {}
    ...
    def dataframe_post_processing(self, df):
        ...
        df = df.rename(columns=self.output_columns)
        return df
...
class DownloadLinksFromDataFrame:
    def __init__(self, excel_file):
        ...
        self.scratch_dir = "../_metadata/_json_scratch"
        self.rel_outpath = "../NFHL"
    ...
    def download_a_link(self, row, session):
        ...
        with threading.Lock():
            product_date = self.state_info_df.loc[self.state_info_df["STATE_FIPS"] ==
                                                  state_fips]["POSTING_DATE"].iloc[0]
...
if __name__ == "__main__":
    ...
    print(outpath)
    # outpath = "../_metadata/state_nfhl_info.xlsx"
